...
/**
 * Đảm bảo bảng badges có đủ các huy hiệu (không cần chạy tay file .sql).
 * INSERT IGNORE: không ghi đè dòng đã có, chỉ thêm mã mới.
 */
const BADGE_ROWS = [
...
async function seedBadges() {
  ...
  const placeholders = BADGE_ROWS.map(() => '(?, ?, ?, ?, ?)').join(', ');
  const flat = BADGE_ROWS.flat();
  await pool.query(
    `INSERT IGNORE INTO badges (code, name, description, icon, xp_required) VALUES ${placeholders}`,
    flat
  );
}
